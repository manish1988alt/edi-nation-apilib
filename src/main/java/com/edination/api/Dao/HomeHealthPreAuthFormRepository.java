...
public interface HomeHealthPreAuthFormRepository extends JpaRepository<HomeHealthPreAuthorizationForm,String> {
    ...
    @Query(value = "SELECT a.* FROM (SELECT * FROM home_health_pre_authorization_form c WHERE c.mrn_number = ?1 ORDER BY c.id DESC ) a WHERE a.id= (SELECT max(e.id) FROM home_health_pre_authorization_form e WHERE e.mrn_number = ?1) ",nativeQuery = true)

...
public interface PreAuthorizationResponseRepository extends JpaRepository<PreAuthorizationResponse,Integer> {
    ...
    @Query(value = "SELECT a.* FROM (SELECT * FROM pr_authorization_response c WHERE c.mrn_number = ?1 ORDER BY c.id DESC ) a WHERE a.id= (SELECT max(e.id) FROM pr_authorization_response e WHERE e.mrn_number = ?1) ",nativeQuery = true)
